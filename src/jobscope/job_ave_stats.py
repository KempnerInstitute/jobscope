...
def slurm_host_map(metrics, duration: int) -> Dict[str, dict]:
    """Slurm's CPU and memory accounting, in the blob's per-node shape.
    ...
    ``total_time`` here is CPU-seconds, matching the blob's per-node field of that name
    -- the top-level ``total_time`` is wall clock. That overload is the blob's, not
...
def accounted(metrics, record: JobRecord) -> bool:
    ...
    ran 1h29, whose blob reports CPU% 6, and ``sstat`` returning no rows at all.
...
def apply_slurm_host(records: Dict[str, JobRecord], jobids,
    ...
    For the site whose CPU%/MEM% would otherwise be blank: no jobstats blob, and no
    cgroup exporter for Prometheus to read.

    ``override`` when the preference puts slurm ahead of the blob, in which case it
    *replaces* the host fields rather than only filling gaps -- naming a source has to
    ...
    rather than quietly reverting to the blob, because a header line claiming slurm over
    blob-derived numbers would be worse than an honest gap.
    """
    ...
    for jid in pending:
        ...
        if override:
            ...
            # CPU% with a blob-derived one. The GPU maps stay: they are a different axis.
