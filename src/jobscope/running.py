...
moment" rather than "how did this job do overall". ``--runtime-avg`` folds over each job's
...
from .slurm import JobRecord, expand_nodelist, run_capture
...
def discover_gpus(client: PrometheusClient, jobs: Dict[int, RunningJob],
    ...
    for entry in series:
        ...
        if not uuid or minor is None:
            continue
        host = config.host_of(labels)
        if not _on_job_nodes(jobs[jobid], host, timeout):
            # The card says it belongs to this job and Slurm says the job is not on that
            # node. Believing the card attributes another job's work to this one -- a
            # wrong number, where dropping it is only a missing one. Measured here while
            # the exporter's mapping was stale: job 37053536 ran on holygpu8a03401 and a
            # card on holygpu8a09201 claimed it.
            misplaced += 1
            continue
        try:
            found.append((uuid, jobid, host, int(minor), labels.get("name", "")))
        except ValueError:
            continue

    if misplaced:
        print("note: ignored %d GPU(s) that claimed a job Slurm places on another node --\n"
              "      %s disagrees with the job's node list, so those cards would have\n"
              "      reported another job's work. Run 'jobscope probe --coverage' to check\n"
              "      whether that mapping is stale." % (misplaced, config.gpu_join()),
              file=sys.stderr)

    # Hoisted, not inlined as the filter expression: there it is rebuilt once per job,
    # which is O(jobs x cards) -- measured at 294ms for `jobscope running -a` on this
    # cluster (7000 jobs, 1700 cards) against 0.3ms for this.
    claimed = {jobid for _u, jobid, *_rest in found}
    unclaimed = {raw for raw in jobs if raw not in claimed}
    recovered = _fallback_gpus(client, jobs, unclaimed, timeout)
    if recovered:
        print("note: %d job(s) resolved through the %s label instead -- %s named no card "
              "for them.\n      Both mappings are live; this one answered."
              % (len({f[1] for f in recovered}), config.gpu_job_label(),
                 config.gpu_join()), file=sys.stderr)
...
def _fallback_gpus(client: PrometheusClient, jobs: Dict[int, RunningJob], unclaimed: set,
                   timeout: Optional[float]) -> List[tuple]:
    """Cards for ``unclaimed`` jobs, from the label-based mapping if a site has one.

    The second mapping described on :class:`jobscope.config.Site`, and the reason it is
    worth having: the two fail independently. When ``nvidia_gpu_jobId`` froze here, every
    job started afterwards resolved no card at all -- and a site also running
    dcgm-exporter's HPC job mapping would have kept reporting, because that label is
    written from Slurm's side rather than inferred from the processes on a card.

    Per job rather than wholesale. A mapping that is merely *behind* still answers
    correctly for older jobs, so replacing its answers would trade a working number for an
    untested one; only the jobs it could not answer are asked again.

    Empty without ``[site] gpu_job_label``, and it costs no query then -- a label nothing
    publishes matches nothing, and paying a round trip per report to learn that is not
    worth it. The host cross-check applies here exactly as above: a second mapping is not
    a more trusted one.
    """
    label = config.gpu_job_label()
    if not (label and unclaimed):
        return []
    series = config.gpu_job_label_series()
    try:
        rows = client.query('%s{%s!=""}' % (series, label), int(time.time()), timeout)
    except Exception:
        return []          # the primary's answer stands; a failed fallback adds nothing
    out = []
    for entry in rows:
        labels = entry.get("metric", {})
        try:
            jobid = int(float(labels.get(label)))
        except (TypeError, ValueError):
            continue
        if jobid not in unclaimed:
            continue
        gpu = _dcgm.gpu_from_series(labels)
        if gpu is None:
            continue
        if not _on_job_nodes(jobs[jobid], gpu["node"], timeout):
            continue
        try:
            out.append((gpu["uuid"], jobid, gpu["node"], int(gpu["minor"]), gpu["model"]))
        except ValueError:
            continue
    return out


_JOB_NODES: Dict[str, frozenset] = {}


def _on_job_nodes(job: RunningJob, host: str, timeout: Optional[float]) -> bool:
    """Whether ``host`` is one of the nodes Slurm says this job holds.

    **True when the node list cannot be determined.** A cross-check that cannot run has
    to weaken to "no opinion", never to a wrong opinion about which host a card belongs
    to: an unexpandable list, or a site that does not report one, would otherwise blank
    every GPU column in the report.

    Memoised by the node list itself rather than by job, so the elements of an array job
    -- which share one -- resolve it once between them. Module-level, following
    ``slurm._HOSTLIST``, rather than threaded through two signatures as a parameter.
    """
    nodelist = str(job.get("node") or "")
    if nodelist not in _JOB_NODES:
        _JOB_NODES[nodelist] = frozenset(
            config.short_host(node) for node in expand_nodelist(nodelist, timeout))
    allowed = _JOB_NODES[nodelist]
    return not allowed or config.short_host(host) in allowed


def requested_gpus(job: RunningJob) -> bool:
    """Whether Slurm records a GPU in this job's allocation.

    ``%b`` is ``tres-per-node``, e.g. ``gres/gpu:1`` -- the allocation as Slurm has it,
    independent of anything Prometheus knows. That independence is the point: it is what
    distinguishes a job with no GPU columns because it asked for no GPU from one whose
    cards could not be found.
    """
    return "gpu" in str(job.get("gres") or "").lower()


def note_missing_gpu_join(jobs: Dict[int, RunningJob], gpus: Dict[str, "Gpu"]) -> None:
    """Say why the GPU columns are blank for jobs that *do* hold GPUs.

    The counterpart of :func:`note_missing_host_series` for the other axis, and the gap
    that prompted it. :func:`discover_gpus` matches a card to a job through the join
    series alone, so when the exporter's mapping goes stale a job holding four H200s
    resolves no cards and every GPU cell prints ``-``. The host columns get an
    explanation; these got a row of dashes that reads as an idle job, or as a jobscope
    bug.

    Measured on this cluster when it happened: 14 of 43 running jobs on one partition,
    all of them holding GPUs, none named by any card. The mapping had stopped ingesting
    new jobs, so every job started after it froze was affected -- one cause, so this is
    said once per report rather than once per job.

    Judged on the *allocation* rather than on ``record.gpus``, which is itself derived
    from what was discovered: asking Prometheus why Prometheus found nothing cannot
    distinguish a CPU-only job from a lost one.
    """
    served = {gpu.jobid for gpu in gpus.values()}
    blind = [job for raw, job in jobs.items()
             if requested_gpus(job) and raw not in served]
    if not blind:
        return
    print("note: %d running job(s) hold GPUs that no card reports -- every GPU column is\n"
          "      blank for them, which is a gap in the telemetry and not an idle job.\n"
          "      %s is the only job-to-card mapping, and nothing in it names these jobs.\n"
          "      Run 'jobscope probe --coverage' to see whether that mapping is stale."
          % (len(blind), config.gpu_join()), file=sys.stderr)
...
def clip_to_job(spec: MetricSpec, raw_jobid: int) -> Optional[str]:
    ...
    One line, delegating: the sacct path went without this rule for as long as it was
    written down only here, and reported one job's GPU% from a previous owner of the card
    (see :func:`jobscope.dcgm.ownership_clip`). Two copies is how that happens again.
    """
    return ownership_clip(spec.uuid_label, raw_jobid)
...
def aggregate_by_job(metrics: RunningMetrics, specs: List[MetricSpec],
    ...
    for raw_jobid, by_uuid in metrics.items():
        ...
        headed = by_header(keyed, specs)
        headed.update({d.header: keyed[d.key] for d in derived if d.key in keyed})
        per_job[raw_jobid] = headed
    return per_job


def by_header(keyed: dict, specs: List[MetricSpec]) -> Dict[str, Optional[float]]:
    """One unit's values re-keyed from metric key to column header.

    The collectors key by ``spec.key`` and everything downstream reads headers, so this
    hop happened in three places with three chances to drift.
    """
    return {spec.header: keyed[spec.key] for spec in specs if spec.key in keyed}
...
def per_gpu_by_node_minor(metrics: RunningMetrics, gpus: Dict[str, Gpu],
    ...
    for uuid, gpu in gpus.items():
        ...
        if not keyed:
            continue
        values = by_header(keyed, specs)
        values.update({d.header: keyed[d.key] for d in derived if d.key in keyed})
        values[MODEL_KEY] = gpu.model     # for the per-architecture POWER_W floor
        out[gpu.jobid][(gpu.host, str(gpu.minor))] = values
    return out


def per_node_pooled(metrics: RunningMetrics, gpus: Dict[str, Gpu],
                    specs: List[MetricSpec],
                    ) -> Dict[int, Dict[str, Dict[str, Optional[float]]]]:
    """Per-node figures for ``--per-node``, pooled from the per-UUID values.

    The counterpart of :func:`per_gpu_by_node_minor` one level up, and pooled from the
    same UUID-keyed source rather than from that function's output: ``(node, minor)``
    collapses MIG siblings onto one key, so reducing it would quietly report a partitioned
    card as one of its instances. :func:`jobscope.dcgm.pool_uuids` does the reduction, so
    a node figure here and on the finished path are the same arithmetic.

    Derived columns are computed *after* pooling, for the reason ``host_series`` gives:
    ``GMEM%`` is used/total, so a node's is the ratio of its summed halves and not the
    mean of its cards' ratios.

    ``MODEL_KEY`` is carried only when every card on the node is the same model. POWER_W's
    floor is per architecture, and a mixed node has no single answer -- empty falls back
    to the global floor, which is the honest reading rather than one card's.
    """
    by_job_node: Dict[int, Dict[str, Dict[str, dict]]] = defaultdict(lambda: defaultdict(dict))
    models: Dict[Tuple[int, str], set] = defaultdict(set)
    for uuid, gpu in gpus.items():
        keyed = metrics.get(gpu.jobid, {}).get(uuid)
        if not keyed:
            continue
        by_job_node[gpu.jobid][gpu.host][uuid] = by_header(keyed, specs)
        models[(gpu.jobid, gpu.host)].add(gpu.model)

    out: Dict[int, Dict[str, Dict[str, Optional[float]]]] = defaultdict(dict)
    for jobid, nodes in by_job_node.items():
        for host, per_uuid in nodes.items():
            pooled = _dcgm.pool_and_derive(per_uuid, specs, list(per_uuid))
            seen = models[(jobid, host)]
            pooled[MODEL_KEY] = next(iter(seen)) if len(seen) == 1 else ""
            out[jobid][host] = pooled
    return out


def host_stats_for(jobs: Dict[int, RunningJob], client: PrometheusClient,
                   timeout: Optional[float] = None) -> Dict[int, dict]:
    """The cgroup half of the summary for a whole selection, in one batched call.

    Split out of :func:`running_records` so a caller that renders in batches pays for it
    once rather than once per batch. The queries are grouped over every job in the
    argument, which is why per-batch calls multiplied them -- measured on a 110-job
    partition, 4 became 56.

    Staleness is not a concern: CPU%/MEM% are cumulative counters (CPU-seconds over
    elapsed x cores, and peak RSS), so reading them at the start of a sweep rather than
    part-way through it is the same number a moment earlier -- and is what the unbatched
    path always did.
    """
    at = int(time.time())
    elapsed_by_job = {raw: job["elapsed_seconds"] for raw, job in jobs.items()
                      if (job.get("elapsed_seconds") or 0) > 0}
    return host_stats_many(elapsed_by_job, at, client, timeout)
...
def running_records(jobs: Dict[int, RunningJob], gpus: Dict[str, Gpu],
                 ...
                 hosts: Optional[Dict[int, dict]] = None) -> Dict[str, JobRecord]:
    ...
    instant-versus-``--runtime-avg`` choice instead of silently re-querying a window.
    """
    ...
    # values already collected, so it honours the instant-versus---runtime-avg choice.
    # ``hosts`` pre-fetched by a batched caller, which is how the grouped cgroup queries
    # stay one set for the selection instead of one per batch. See host_stats_for.
    if hosts is None:
