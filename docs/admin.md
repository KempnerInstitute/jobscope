...
There used to be a `--validate` that put one job's utilization side by side as
Prometheus, the jobstats summary and Slurm's own accounting each measured it. It is
gone, because its GPU row was not a comparison. Slurm's `gres/gpuutil` is a single
point reading rather than an average over the job -- `TRESUsageInTot`,
`InAve`, `InMax` and `InMin` all return the same number -- so that row compared a
snapshot against a whole-run mean, and the size of the disagreement tracked how fast
the metric was moving rather than anything about accuracy. Measured on one bursty job:
Slurm 82, Prometheus 51, and the duty cycle moving a median of 67 points between
consecutive scrapes. On steady jobs the same comparison agreed to within 5 points.

Two exporters measuring the same quantity is the comparison it was reaching for:
run the same report under `--gpu-source nvml` and `--gpu-source dcgm`, and use
`--verify`'s `SWING` column to see whether either mean is reproducible at all. For
the host columns, `[host] source = slurm` still reads Slurm's own accounting, where
...
`[promtheus]` used to cost a site every setting under it in silence; it now says so. A
note rather than an error, because an unread section is inert and the rest of the file
still resolves.
