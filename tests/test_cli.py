...
# --- reporting on other users -----------------------------------------------

def _cfg_with_group(group):
    """A config whose admin_group is `group`, leaving everything else alone."""
    import dataclasses as dc

    from jobscope import config
    base = config.get_config()
    return dc.replace(base, defaults=dc.replace(base.defaults, admin_group=group))


def test_all_users_needs_the_admin_group(monkeypatch):
    monkeypatch.setattr(cli, "in_group", lambda name: False)
    monkeypatch.setattr(cli, "default_user", lambda: "alice")
    with pytest.raises(JobscopeError) as exc:
        build_request(_args(all_users=True, user=None), _cfg_with_group("slurm-admin"))
    message = str(exc.value)
    assert "-a/--all-users" in message and "slurm-admin" in message


def test_another_user_needs_the_admin_group(monkeypatch):
    monkeypatch.setattr(cli, "in_group", lambda name: False)
    monkeypatch.setattr(cli, "default_user", lambda: "alice")
    with pytest.raises(JobscopeError) as exc:
        build_request(_args(user="bob"), _cfg_with_group("slurm-admin"))
    message = str(exc.value)
    assert "bob's jobs" in message
    # The message must offer the way forward, not just refuse.
    assert "-u alice" in message


def test_naming_yourself_is_always_allowed(monkeypatch):
    monkeypatch.setattr(cli, "in_group", lambda name: False)
    monkeypatch.setattr(cli, "default_user", lambda: "alice")
    assert build_request(_args(user="alice"), _cfg_with_group("slurm-admin")).user == "alice"


def test_group_members_may_ask_for_anyone(monkeypatch):
    seen = []
    monkeypatch.setattr(cli, "in_group", lambda name: seen.append(name) or True)
    monkeypatch.setattr(cli, "default_user", lambda: "alice")
    assert build_request(_args(all_users=True, user=None),
                         _cfg_with_group("slurm-admin")).all_users
    assert build_request(_args(user="bob"), _cfg_with_group("slurm-admin")).user == "bob"
    assert seen == ["slurm-admin", "slurm-admin"]   # the configured group, not a literal


def test_an_empty_admin_group_disables_the_gate(monkeypatch):
    def boom(name):
        raise AssertionError("no group lookup when the gate is disabled")

    monkeypatch.setattr(cli, "in_group", boom)
    monkeypatch.setattr(cli, "default_user", lambda: "alice")
    assert build_request(_args(all_users=True, user=None), _cfg_with_group("")).all_users


def test_explicit_jobids_are_not_gated(monkeypatch):
    """A JOBID bypasses the filters, so there is nothing for the gate to widen.

    Looking up a colleague's job by ID stays possible -- which is how you help
    someone debug -- and sacct would show it to them anyway.
    """
    monkeypatch.setattr(cli, "in_group", lambda name: False)
    monkeypatch.setattr(cli, "default_user", lambda: "alice")
